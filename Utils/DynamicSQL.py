...
# def insert_into_table_from_local_shp_sql(table_name, field_from_form, feature, attribute_info_list):
#     # 构建 CREATE TABLE 语句的起始部分
...
def get_create_spreadsheet_sql(shpName, table_name):
    cases = {
        "FlowRate": f"CREATE TABLE IF NOT EXISTS {table_name} (Fid serial PRIMARY KEY,name VARCHAR,time TIMESTAMP,flow double precision,waterlevel double precision)",
        ...
        "GeologicalBody": f"CREATE TABLE IF NOT EXISTS {table_name} (Fid serial PRIMARY KEY,name VARCHAR,layer integer,BottomElevation integer,OrificeElevation integer)",
        "HRU": f"CREATE TABLE IF NOT EXISTS {table_name} (Fid serial PRIMARY KEY,name VARCHAR,zero double precision,nimp double precision,nperv double precision,simp double precision,sperv double precision,raingage VARCHAR,outfall VARCHAR,area double precision,curblen double precision,width double precision,slope double precision,pctzero double precision,routeto VARCHAR)",
        "RainGauge": f"CREATE TABLE IF NOT EXISTS {table_name} (Fid serial PRIMARY KEY,name VARCHAR,format VARCHAR,intvl VARCHAR,scf VARCHAR,datasource VARCHAR,sourcename VARCHAR)",
        "Junction": f"CREATE TABLE IF NOT EXISTS {table_name} (Fid serial PRIMARY KEY,name VARCHAR,invertelevation double precision,mdepth double precision,idepth double precision,sdepth double precision,pondedarea double precision)",
        "MODFLOW_InitialWaterLevel": f"CREATE TABLE IF NOT EXISTS {table_name} (Fid serial PRIMARY KEY,name VARCHAR,layer integer,initialhead double precision)",
        "EFDC_Parameter": f"CREATE TABLE IF NOT EXISTS {table_name} (Fid serial PRIMARY KEY,chd VARCHAR,smagcoef VARCHAR,backvisc VARCHAR,maxlamvisc VARCHAR,maxturbdiffcoef VARCHAR,rough VARCHAR)",
        "SurfaceRive_Point": f"CREATE TABLE IF NOT EXISTS {table_name} (Fid serial PRIMARY KEY,name VARCHAR,time TIMESTAMP,riverlevel double precision,riverwidth double precision,riverbedelevation double precision,riverbedthickness double precision,riverdiversion double precision)",
    }
...
# 动态生成创建表的SQL语句
def generate_create_table_sql_from_shp(table_name, field_info_list):
    """
    动态生成创建表的SQL语句。
    ...
    for item in field_info_list:
        ...
        if item.get('name').lower() != "fid" and item.get('name').lower() != "object" and item.get(
                'name').lower() != "objectid":
            ...
            # 根据字段信息添加到 SQL 语句中
            sql_statement += f"{item.get('name').lower()} {sql_type}"
...
def insert_into_table_from_shp_sql(table_name, feature, attribute_info_list):
    # 构建 CREATE TABLE 语句的起始部分
    sql_statement = f"INSERT INTO {table_name}("
    for field_info in attribute_info_list:
        for key, value in field_info.items():
            if key == "name" and value.lower() != "fid" and value.lower() != "object" and value.lower() != "objectid":
    ...
    for field_info in attribute_info_list:
        for key, value in field_info.items():
            if key == "name" and value.lower() != "fid" and value.lower() != "object" and value.lower() != "objectid":
                type_value = field_info.get('type')
                if type_value == 'String':
                    sql_statement += f"'{feature.GetField(value)}', "
                else:
                    sql_statement += f"{feature.GetField(value)}, "
...
def get_relationship_between_attribute_and_fields_from_spreadsheet(sql):
    column_pattern = r"\((.*?)\)"  # 匹配括号内的内容
    column_string = re.search(column_pattern, sql).group(1)
    columns = [column.strip() for column in column_string.split(",")]
    column_dict = {}
    ...
    for column in columns:
        ...
        column_dict[column_name] = column_type

    return column_dict


def insert_into_table_from_spreadsheet_sql(data, table_name, values):
    sql_statement = f"INSERT INTO {table_name}("

    for field_info in values:
        if field_info.lower() != 'fid':
            sql_statement += f"{field_info}, "
    ...
    for key, value in values.items():
        if key.lower() != 'time' and key.lower() != 'fid':
            dataValue = data[index]
            if value == "character varying" or value == "VARCHAR":
                sql_statement += "'" + str(dataValue) + "'" + ","
            else:
                sql_statement += str(dataValue) + ","
        elif key.lower() == 'time':
...
def update_into_table_from_body_sql(table, data):
    # 构建 SET 子句
    set_clause = ', '.join([f"{key} = {repr(value)}" for key, value in data.items()])

    # 构建动态的 UPDATE 语句
    sql = f"UPDATE {table} SET {set_clause} WHERE fid = {data['fid']}"
    return sql
